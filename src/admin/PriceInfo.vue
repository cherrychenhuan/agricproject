      ...
      <el-row>
        ...
        <el-col :span="5">
          <el-form-item :label="'日期:'">
            <el-date-picker v-model="filterData1.date" type="date" placeholder="" />
    ...
    <el-row>
      <el-col :span="14">
        <el-table id="profuctTable" style="margin-top:0;height:72.8vh;" @select="setSelectRows" :row-key="getRowKey"
          ...
          <el-table-column align="center" :show-overflow-tooltip="true" prop="date" label="日期" width="90" />
          <!-- <el-table-column align="center" prop="marketcode" :show-overflow-tooltip="true" label="企业编码" width="60" /> -->
          <el-table-column align="center" prop="marketname" :show-overflow-tooltip="true" label="企业名称" width="150" />
          <el-table-column align="center" prop="province" label="省份" width="125" />
          <el-table-column align="center" prop="priceMarket" label="市场价格" width="90" />
      ...
      <el-col :span="10">
        ...
        <el-form ref="productForm" :show-message="false" :rules="rules" class="dialog-form" @submit.native.prevent=""
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="日期:" style="margin-left:0;">
                <el-date-picker v-model="productData.date" type="date" placeholder="" />
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="企业代码:" style="margin-left:0;">
                <el-input v-model="productData.marketcode" />
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="企业:" style="margin-left:0;">
                <el-input v-model="productData.marketname" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item label="价格:" style="margin-left:0;">
                <el-input v-model="productData.priceMarket" />
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="日环比:" style="margin-left:0;">
                <el-input v-model="productData.priceMarketRate" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            ...
            <el-col :span="24">
              <el-form-item label="经度:" style="margin-left:0;">
                <el-input v-model="productData.longitude" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            ...
            <el-col :span="24">
              <el-form-item label="纬度:" style="margin-left:0;">
                <el-input v-model="productData.latitude" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
...
import { deletePriceInfoService, updatePriceDataService, getNewPriceService, addPriceDataService, filterPriceInfoService, deleteProductInfoService, getPriceDataService, addProductDataService, updateProductDataService, getNewProductService } from '@/api/admin.js'
...
const saveProductInfo = async () => {
  ...
  xxdata.date = formatDate(xxdata.date)
  if (editState1.value == 'add') {

    let result = await addPriceDataService(xxdata)
    if (result.data == '已经存在') {
    ...
    for (let i = 0; i < productList.value.length; i++) {
      ...
      if (item.cropname == xxdata.cropname && item.date == xxdata.date && item.marketname == xxdata.marketname) {
        itemRow = item;
        rowIndex = i;
        break;
      }
...
const filterProductInfo = async () => {
  ...
  xxdata.date = formatDate(xxdata.date)
  let result1 = await filterPriceInfoService(xxdata, pageNumProduct.value, pageSizeProduct.value)
...
const getProductList = async (flag, row) => {
  try {
    ...
    let result = await getPriceDataService(params);
    //渲染视图
...
const deleteProductInfo = async (rowData) => {
  try {
    let result = await deletePriceInfoService(rowData);
...
const rowlineDeleteBtn1 = async (rowData) => {
  ElMessageBox.confirm(
    '确认删除' + rowData.date + rowData.marketname + '的' + rowData.cropname + '市场数据吗?',
    '提示',
    {
    ...
    if (action === 'confirm') {
      productList.value.forEach((v, i) => {
        if (v.cropname == rowData.cropname && v.date == rowData.date && v.marketname == rowData.marketname) {
